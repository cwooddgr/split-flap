...
Current version: **1.2** (build 3; tag `tvos-1.2-3`), **live on the App Store since 2026-09-21** (submitted 8:36 AM MDT that day and approved the same afternoon; `READY_FOR_SALE` read from the App Store Connect API at 4:07 PM MDT, and the public App Store lookup shows 1.2 with a release date of 2:26 PM MDT). Before it, 1.1 (build 3, uploaded 2026-07-09) was live from 2026-07-10; our docs said build 2 until 2026-09-21, but build 2 is an unreleased 1.1 upload from 2026-05-14. No version is in review now. To read the state again: `node ../otd/scripts/asc.mjs GET "/v1/apps/6761391738/appStoreVersions?limit=3"` (app id 6761391738; the API keeps showing `READY_FOR_SALE` on 1.1 and 1.0 too, so check the public lookup for which one is live); a rejection goes in `../APP_STORE_REJECTIONS.md` and is never resubmitted unasked. The 1.2 version record has the build attached, 1.1's promotional text copied over unchanged (a new version does not inherit it), a What's New Charlie approved as written, and the corrected Apple TV Privacy Policy text (all decided-by-user 2026-09-21, along with the submission itself and his word that the day on his Apple TV went fine). Build numbers are per version on tvOS, so 1.1 (3) and 1.2 (3) are different binaries. The submission itself was three App Store Connect calls (POST `/v1/reviewSubmissions`, POST `/v1/reviewSubmissionItems`, PATCH `submitted: true`); on 2026-09-21 the auto mode classifier refused them from the agent, and Charlie got them through himself. Later that day he stated a standing permission for these calls (see "tvOS release process" below). What 1.2 holds beyond 1.1: the audio session is set to `.ambient` at launch so the app no longer stops the user's music (1.1 did, at the first flap; confirmed on a device), the audio engine restarts after a route change (never reproduced), and the layout is the new fixture-tested port (breaks long words across lines, folds accents, no triple spaces). Also in 1.2, on `main` since 2026-09-20 (scope decided-by-user that day, his items "1-4 plus 5 and 8"): the connection scaffolding in `RoomViewModel.swift` is gone (sign-in and the listener retry with backoff, one listener, never a sign-out), Firestore uses a memory-only cache, the Canvas draws from a main-thread copy of the board state, the QR image is built once, a "Reconnecting…" line shows at the bottom left after 5 seconds of trouble, and everything on screen drifts around a 6-point circle, one step every 3 minutes, against burn-in. **The QR code stays up until the user hides it; don't add an auto-hide.** Built and run in the simulator only, apart from the music fix, which Charlie confirmed on his Apple TV. Bundle ID `co.dgrlabs.flipflap`, deployment target tvOS 17.0. Bump `MARKETING_VERSION` and `CURRENT_PROJECT_VERSION` in `tvos/SplitFlapTV/Version.xcconfig`, the one place they live (since 2026-09-20 it is the project's base configuration for Debug and Release; the pbxproj no longer sets either).
...
### tvOS release process

> **Author:** Claude Code (coder) · **Date:** 2026-09-21 · **Status:** the permission is decided-by-user (quoted below); the steps are proposed-by-agent and are what worked on 1.2.

Charlie, 2026-09-21, for all DGR Labs app projects (also in the workspace `CLAUDE.md` and as an `autoMode.allow` rule in `~/.claude/settings.json`): "Standing permission: in my app projects, the project's agent may submit a version to App Review through the App Store Connect API once I've told it in that session to submit that version. It never resubmits after a rejection without asking me." And, when I suggested he name the app each time he says to submit: "I do NOT want to have to name the app when I'm working in its project directory. which app should be obvious." In this directory the app is Flip Flap, and the version is the one the session has been preparing.

1. **Cut.** Read `../APP_STORE_REJECTIONS.md`. Run `node --test tests/layout.test.mjs` and `cd tvos && swift test`. Check the build numbers already on App Store Connect (they are per version on tvOS, but pick one no earlier build has used), bump `tvos/SplitFlapTV/Version.xcconfig`, commit, tag `tvos-<version>-<build>`, push.
2. **Upload.** `xcodebuild -project tvos/SplitFlapTV/SplitFlapTV.xcodeproj -scheme SplitFlapTV -configuration Release -destination 'generic/platform=tvOS' -archivePath <scratch>/FlipFlap.xcarchive -allowProvisioningUpdates archive`, then `xcodebuild -exportArchive` with an export options plist of `method` app-store-connect, `destination` upload, `signingStyle` automatic, `teamID` 2CTUXD4C44. Automatic signing goes through the Apple ID signed in to Xcode, so this step needs no API key. Keep the archive out of the repo. Wait for the build to read `VALID`.
3. **Prepare.** The API client is `node ../otd/scripts/asc.mjs <METHOD> <path> [json|@file]`. App id `6761391738`, platform `TV_OS`. `POST /v1/appStoreVersions` with the previous version's `releaseType`; `PATCH` `whatsNew` on the new version's en-US localization with text Charlie has approved (keep a copy in `APP_STORE_LISTING.md`); copy `promotionalText` from the previous version, because a new version does not inherit it (null on 1.2); `PATCH /v1/appStoreVersions/{id}/relationships/build`. The App Review contact and notes carry over. App-level text such as the Apple TV Privacy Policy lives on the `appInfoLocalization` of the app info in `PREPARE_FOR_SUBMISSION`, not the live one.
...
History: until 2026-09-20 the ports disagreed (the Swift one turned a double space into a triple space on a wrapping line, and both uppercased after wrapping so `ß` could overflow). tvOS 1.1 and earlier have the old Swift layout; the new one shipped in 1.2 (live 2026-09-21). The web display got the new layout the day it was pushed.
